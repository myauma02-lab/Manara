...
export default function IncomePage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    financeApi.transactions({ type: "INCOME", status: filterStatus || undefined, category: filterCat || undefined, search: search || undefined, dateFrom: dateFrom || undefined, dateTo: dateTo || undefined, page, limit: 15 })
      .then(r => {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", backdropFilter: "blur(4px)" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: "12px", width: "100%", maxWidth: "480px", overflow: "hidden", boxShadow: "0 24px 80px rgba(0,0,0,0.2)" }}>
        <div style={{ background: `linear-gradient(135deg,#1a1208,${form.type === "INCOME" ? "#3F6F6A" : "#f87171"})`, padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <form onSubmit={handleSubmit} style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          {form.amount && (
            <div style={{ background: `rgba(${form.type === "INCOME" ? "63,111,106" : "248,113,113"},0.06)`, border: `1px solid rgba(${form.type === "INCOME" ? "63,111,106" : "248,113,113"},0.15)`, borderRadius: "6px", padding: "10px 14px", display: "flex", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            ...
            <button type="submit" disabled={saving} style={{ flex: 2, padding: "12px", background: saving ? "#B8CDD2" : (form.type === "INCOME" ? "#3F6F6A" : "#f87171"), color: "#fff", border: "none", borderRadius: "6px", fontSize: "13px", fontWeight: 500, cursor: saving ? "not-allowed" : "pointer" }}>
